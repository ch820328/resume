import os
import re

slides_dir = 'src/slides'

def fix_html_split_layout(file_path):
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()

    # Skip special slides
    if 'id="slide-1"' in content or 'id="slide-end"' in content:
        return

    # Extract components
    section_match = re.search(r'<section class="slide" id="(.*?)">', content)
    if not section_match: return
    slide_id = section_match.group(1)

    title_match = re.search(r'<h2 class="slide-title">(.*?)</h2>', content)
    slide_title = title_match.group(1) if title_match else ""

    tag_match = re.search(r'<span class="slide-tag">(.*?)</span>', content)
    slide_tag = tag_match.group(1) if tag_match else ""

    cards = re.findall(r'(<div class="star-card">.*?</div>)', content, re.DOTALL)
    tech_tags = re.findall(r'<span class="tech-tag">(.*?)</span>', content)
    
    # Improved visual extraction - find image/svg within star-visual
    visual_match = re.search(r'<div class="star-visual">.*?<img (.*?)>.*?</div>', content, re.DOTALL)
    if visual_match:
        img_attr = visual_match.group(1)
        visual_content = f'<img {img_attr}>'
    else:
        # Fallback to simple content search
        visual_match = re.search(r'<div class="star-visual">(.*?)</div>', content, re.DOTALL)
        visual_content = visual_match.group(1).strip() if visual_match else ""

    # Clean up cards (they might have nesting issues from previous run)
    clean_cards = []
    for card in cards:
        # Extract the star-label and star-text specifically
        label_match = re.search(r'<span class="star-label">(.*?)</span>', card)
        text_match = re.search(r'<span class="star-text">(.*?)</span>', card, re.DOTALL)
        if label_match and text_match:
            label = label_match.group(1)
            text = text_match.group(1).strip()
            clean_cards.append(f"""                <div class="star-card">
                    <div class="star-item">
                        <span class="star-label">{label}</span>
                        <span class="star-text">
                            {text}
                        </span>
                    </div>
                </div>""")

    # Reconstruct with split layout
    new_content = f"""<section class="slide" id="{slide_id}">
    <div class="content-wrapper">
        <div class="slide-header-row">
            <h2 class="slide-title">{slide_title}</h2>
            <span class="slide-tag">{slide_tag}</span>
        </div>
        <div class="star-grid">
            <div class="star-content">
"""
    for card in clean_cards:
        new_content += f"{card}\n"
    
    new_content += """                <div class="tech-footer">
"""
    for tag in tech_tags:
        new_content += f'                    <span class="tech-tag">{tag}</span>\n'
    
    new_content += f"""                </div>
            </div>
            <div class="star-visual">
                <div class="process-flow-viz">
                    {visual_content}
                </div>
            </div>
        </div>
    </div>
</section>"""

    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print(f"✅ Fixed {file_path}")

# Run on all slides
for filename in os.listdir(slides_dir):
    if filename.endswith('.html'):
        fix_html_split_layout(os.path.join(slides_dir, filename))
